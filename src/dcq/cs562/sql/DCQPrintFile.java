...
public class DCQPrintFile {
	...
	// 6 Emfvariables 
	private DCQMFVariables varibles;
	// import String list
	...
	public DCQMFVariables getVaribles() {
		return varibles;
	}
	public void setVaribles(DCQMFVariables varibles) {
		this.varibles = varibles;
	}
	public DCQPrintFile(String fileName, String packageName,
...
//	appent the content of  table class to returnValue 
...
//	appent the content of MFStruture class to returnValue 
	public void printMFStruture(DCQConnector connector){
		returnValue.append("\r\n class MFStruture{\r\n");
		...
		for (int j = 0; j < S.size(); j++) {
			...
			if(!(attrubutes.contains(name))&&!(name.contains("_"))){
				String type = connector.getTableStructure().get(name);
				attrubutes.add(name);
				returnValue.append("\tprivate " + type + " " + name + ";\r\n");
				String 	upperCaseString =  name.substring(0, 1).toUpperCase() + name.substring(1);
				//set function
...
//	appent the content of  comments part to returnValue 
...
//	appent the content of  package part to returnValue
...
//	appent the content of  import part to returnValue
...
//	appent the content of  databse part to returnValue, this part is a part of printClass
	void printDatabaseConnt(DCQConnector connector){
		StringBuffer database = new StringBuffer();
		 Properties prop =  connector.getProp();
...
//	appent the content of MF-strucute initial to returnValue, this part is a part of printClass
	public void printFirstScan(DCQConnector connector){
		// we initial a variable and append it at last
		StringBuffer firstScan = new StringBuffer();

		String tableNameString = connector.getTableName();
		...
		for (int j = 0; j < tupleV.size(); j++) {
			...
			if(connector.getTableStructure().get(tupleV.get(j)).equals("int")){
		...
		for (int i = 0; i < this.mfvaribles.size(); i++) {
			...
			if(mfvariable.contains("0_")){
			...
			}else if(connector.getTableStructure().containsKey(mfvariable)){
				...
				if(mfvariable.contains("avg_")){
					...
					firstScan.append("\tmfstructue.set"+mfcount+"0);\r\n");
				}
				else{
...
//	appent the content of MF-strucute update of each Traversal to returnValue, this part is a part of printClass
//  We need get N, where, sigma, F from mf variables

	private void printHandleMFstructure(DCQConnector connector) {
...
//	appent the content of MF-strucute update(delete) to returnValue, this part is a part of printClass
//	 We need delete some elements of mf-sturcure, base on having clause.
	private void printHavingClause(DCQConnector connector) {
...
//	appent the content of FinalResult to returnValue, this part is a part of printClass
//	we need output the final result, and show it to our user

	private void printFinalResult(DCQConnector connector) {
		...
		for (int i = 0; i < select.size(); i++) {
			...
			length = attr.length();
			if(connector.getTableStructure().containsKey(attr)){
				length = 8;
			}
			if(attr.contains("_")){
			...
			}else if(connector.getTableStructure().containsKey(attr)){
				if(connector.getTableStructure().get(attr).equals("int")){
					finalString.append("\tSystem.out.printf(\"%-"+length+"s\",\""+attr+"\");\r\n");
				}else{
					finalString.append("\tSystem.out.printf(\"%-"+length+"s\",\""+attr+"\");\r\n");
				}
...
//					finalString.append("\tDouble.parseDouble(mf.get"+sum+"())+\"\" / Double.parseDouble(mf.get"+count+"()+\"\")+);\r\n");
				...
				if (attr.contains("avg_")) {
					...
					String agg = "Mf"+attr;
					finalString.append("(int)mf.get"+agg+"());\r\n");
					}
			}else{
				finalString.append("\tSystem.out.printf(\"");
				if(connector.getTableStructure().get(attr).equals("int")){
...
//	such as printDatabaseConnt,printFirstScan,printHandleMFstructure,printHavingClause,printFinalResult
	public void printClass(DCQConnector connector){
		...
		printFirstScan(connector);
		// print the update mf sturcute part
		printHandleMFstructure(connector);
...
//	this part is the entry for this class, during some process, this function will return 
//	returnVaule and that is the reuslt we want to output to file
	public  String write(DCQConnector connector) {
